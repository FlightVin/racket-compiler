"""
https://github.com/YarikTH/pretty.py?tab=readme-ov-file
"""
...
def check_tools(tools: Tools) -> bool:
    ...
    if not is_tool(tools.clang_format):
        result = False
        print(
            f"clang-format tool ({tools.clang_format}) is not found.\n"
    ...
    if not is_tool(tools.gersemi):
        result = False
        print(
            f"gersemi tool ({tools.gersemi}) is not found.\n"
...
def check_files(
    tools: Tools, cxx_files_to_check: List[str], cmake_files_to_check: List[str]
) -> int:
    import subprocess
    ...
    if len(cxx_files_to_check) != 0:
        try:
            subprocess.check_call(
                [tools.clang_format, "--Werror", "--dry-run"] + cxx_files_to_check
            )
...
def prettify_files(
    ...
    if len(cmake_files_to_check) != 0:
        try:
            subprocess.check_call(
                [tools.gersemi, "--in-place", "-q"] + cmake_files_to_check
            )
        except subprocess.CalledProcessError as e:
            result = e.returncode
...
def collect_non_ignored_files(
    working_tree_dir: str, excluded: List[str] = []
) -> List[str]:
    import git
    ...
    for root, dirs, files in os.walk(working_tree_dir):
        ...
        file_entries.extend(
            [os.path.relpath(os.path.join(root, f), working_tree_dir) for f in files]
        )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="A formatter to make your C++ and CMake code the real treasure."
    )
    parser.add_argument(
        "src",
        type=str,
    ...
    parser.add_argument(
        "--gersemi", type=str, default="gersemi", help="Gersemi to use for CMake files"
    )
