...
void ToIRVisitor::run(AST *Tree) {
  FunctionType *MainFty =
      FunctionType::get(LLVMInt32Ty, {}, false); // Use LLVM type
  Function *MainFn =
  ...
  if (Node.getExpr()) {
    ...
    if (!finalV) {
      if (finalType != VoidType::get() && finalType != ErrorType::get()) {
        llvm::errs()
            << "Codegen Error: Final value 'V' is null for non-void type "
    ...
    if (finalType == IntegerType::get()) {
      if (finalV->getType() == LLVMInt1Ty) { // Cast bool to int if necessary
        finalV =
            Builder.CreateZExt(finalV, LLVMInt32Ty, "final_bool2int_for_write");
      } else if (finalV->getType() != LLVMInt32Ty) {
        llvm::errs() << "Codegen Error: Final Integer value has wrong LLVM "
                        "type for write_int: "
                     << *finalV->getType() << "\n";
      ...
      if (finalV->getType() == LLVMInt1Ty) {
        ...
        Value *isNonZero =
            Builder.CreateICmpNE(finalV, LLVMInt32Zero, "tobool_for_write");
        valToWrite = Builder.CreateZExt(isNonZero, LLVMInt32Ty,
                                        "final_bool2int_for_write");
        llvm::errs() << "Codegen Warning: Final Boolean value had i32 type. "
                        "Converting to 0/1 for write_bool.\n";
      } else {
        llvm::errs() << "Codegen Error: Final Boolean value has wrong LLVM "
                        "type for write_bool: "
                     << *finalV->getType() << "\n";
      ...
      llvm::errs()
          << "Codegen: Final expression has Error type. No output generated.\n";
    } else { // Add cases for Vector etc. later
      llvm::errs() << "Codegen: Unhandled final expression type for printing: "
                   << finalType->getName() << "\n";
    }
  } else {
