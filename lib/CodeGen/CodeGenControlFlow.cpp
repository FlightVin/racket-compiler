...
void ToIRVisitor::visit(If &Node) {
  // Access V, Builder, Ctx, ExprTypes, getLLVMType, Type singletons, LLVM
  // constants directly
  ...
  if (CondV->getType() == LLVMInt32Ty) {
    ...
    llvm::errs()
        << "Codegen Error: If condition did not yield an i1 or i32. Yielded: "
        << *CondV->getType() << "\n";
  ...
  if (!ThenV && ResultType != VoidType::get()) { // MODIFIED check VoidType
    llvm::errs()
        << "Codegen Warning: Null value from 'then' branch. Using default.\n";
    ThenV = llvm::Constant::getNullValue(PhiLLVMType);
  }
  // Cast if needed (using LLVM types)
  if (ResultType != VoidType::get() && ThenV &&
      ThenV->getType() != PhiLLVMType) {
    if (PhiLLVMType == LLVMInt32Ty && ThenV->getType() == LLVMInt1Ty)
      ThenV = Builder.CreateZExt(ThenV, LLVMInt32Ty, "then_cast");
    else if (PhiLLVMType == LLVMInt1Ty && ThenV->getType() == LLVMInt32Ty)
      ThenV = Builder.CreateICmpNE(ThenV, LLVMInt32Zero, "then_cast");
    // Add more casts...
    else {
      llvm::errs() << "Codegen Warning: Unhandled type mismatch in 'then' "
                      "branch. Using default.\n";
      ThenV = llvm::Constant::getNullValue(PhiLLVMType);
    }
  ...
  if (!ElseV && ResultType != VoidType::get()) { // MODIFIED check VoidType
    llvm::errs()
        << "Codegen Warning: Null value from 'else' branch. Using default.\n";
    ElseV = llvm::Constant::getNullValue(PhiLLVMType);
  }
  // Cast if needed
  if (ResultType != VoidType::get() && ElseV &&
      ElseV->getType() != PhiLLVMType) {
    if (PhiLLVMType == LLVMInt32Ty && ElseV->getType() == LLVMInt1Ty)
      ElseV = Builder.CreateZExt(ElseV, LLVMInt32Ty, "else_cast");
    else if (PhiLLVMType == LLVMInt1Ty && ElseV->getType() == LLVMInt32Ty)
      ElseV = Builder.CreateICmpNE(ElseV, LLVMInt32Zero, "else_cast");
    // Add more casts...
    else {
      llvm::errs() << "Codegen Warning: Unhandled type mismatch in 'else' "
                      "branch. Using default.\n";
      ElseV = llvm::Constant::getNullValue(PhiLLVMType);
    }
  ...
  if (ResultType != VoidType::get()) { // MODIFIED check VoidType
    ...
    PN->addIncoming(ThenV ? ThenV : llvm::UndefValue::get(PhiLLVMType),
                    ThenEndBB);
    PN->addIncoming(ElseV ? ElseV : llvm::UndefValue::get(PhiLLVMType),
                    ElseEndBB);
    V = PN;
  } else {
...
void ToIRVisitor::visit(WhileLoop &Node) {
  ...
  if (CondV->getType() == LLVMInt32Ty) {
    ...
    llvm::errs() << "Codegen Error: While condition did not yield Boolean (i1 "
                    "or i32).\n";
    CondV = LLVMFalseConstant;
  }
...
void ToIRVisitor::visit(Begin &Node) {
  // Access ExprTypes, getLLVMType, V, Builder, Type singletons, LLVM constants
  // directly
  Type *ResultType = ExprTypes.lookup(&Node); // Get llracket::Type*
  if (!ResultType)
  ...
  if (!lastVal) {
    llvm::errs() << "Codegen Internal Error: lastVal is null after visiting "
                    "non-empty 'begin' block.\n";
  ...
  if (lastVal->getType() != ResultLLVMType) {
    llvm::errs()
        << "Codegen Warning: Type mismatch for final value in Begin. Expected "
        << *ResultLLVMType << ", got " << *lastVal->getType() << ". Casting.\n";
    if (ResultLLVMType == LLVMInt32Ty && lastVal->getType() == LLVMInt1Ty)
      V = Builder.CreateZExt(lastVal, LLVMInt32Ty, "begin_cast");
    else if (ResultLLVMType == LLVMInt1Ty && lastVal->getType() == LLVMInt32Ty)
      V = Builder.CreateICmpNE(lastVal, LLVMInt32Zero, "begin_cast");
    // Add more casts...
    else {
      llvm::errs() << " -- Cannot cast final 'begin' value.\n";
      V = llvm::Constant::getNullValue(ResultLLVMType);
    }
  } else {
