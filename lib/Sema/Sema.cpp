...
bool Sema::typeCheck(AST *Tree) {
  ...
  // Perform final check (implementation might change depending on inference strategy)
...
TypeCheckVisitor::TypeCheckVisitor(DiagnosticsEngine &D,
                                   llvm::DenseMap<Expr *, Type*> &ET, // Note: llracket::Type implied by Sema.h context
                                   llvm::StringMap<Type*> &VT)       // Note: llracket::Type implied by Sema.h context
...
// Takes llracket::Type*
void TypeCheckVisitor::reportTypeError(llvm::SMLoc Loc, Type* Expected, Type* Actual, const std::string &context) {
    ...
    Diags.report(Loc, diag::err_type_mismatch,
                 ...
                 Actual ? Actual->getName() : "<nullptr-actual>",
                 context_msg);
    HasError = true;
}

// Takes llracket::Type*
void TypeCheckVisitor::reportExpectedTypeError(llvm::SMLoc Loc, Type* Expected, Type* Actual, const std::string &context) {
    std::string context_msg = context.empty() ? "" : (" " + context);
...
llracket::Type* TypeCheckVisitor::visitAndGetType(Expr *Node) {
    ...
    if (it != ExprTypes.end()) {
        ...
        return it->second; // Return recorded type (could be ErrorType if sub-visit failed)
    }
