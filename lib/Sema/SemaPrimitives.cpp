...
void TypeCheckVisitor::visit(Prim &Node) {
  ...
  if ((E1 && (!T1 || T1 == ErrorType::get())) ||
    ...
    if ((E1 && !T1) || (E2 && !T2))
      HasError =
          true; // Ensure HasError is set if visitAndGetType returned null
    return;
  }

  // Type checking based on operator, including inference for
  // ReadPlaceholderType
  switch (Node.getOp()) {
  case TokenKind::read:
    if (E1 || E2)
      ...
      resultType = ReadPlaceholderType::get(); // Return placeholder
    break;
      ...
      if (!OpError) {
        if (T1->equals(T2)) { // Check if types are equal *after* inference
          if (T1 != IntegerType::get() && T1 != BooleanType::get()) {
            ...
            reportError(getLoc(E1 ? E1 : &Node), diag::err_invalid_operands,
                        "eq?", "Integer or Boolean, but got " + T1->getName());
            OpError = true;
          }
