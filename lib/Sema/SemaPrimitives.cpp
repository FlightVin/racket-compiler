...
void TypeCheckVisitor::visit(Prim &Node) {
  ...
  if (E3) T3 = visitAndGetType(E3); // Visit E3 if it exists
  ...
  if ((E1 && (!T1 || T1 == ErrorType::get())) ||
    ...
    if ((E1 && !T1) || (E2 && !T2) || (E3 && !T3)) HasError = true;
    return;
  }
  ...
  switch (Node.getOp()) {
  case TokenKind::read:
    if (E1 || E2 || E3) reportOperandCountError(0); // Updated check
    else resultType = ReadPlaceholderType::get();
    break;
  ...
  case TokenKind::vector_length:
      if (E1 && !E2 && !E3) {
          if (T1 == ReadPlaceholderType::get()) {
               reportError(getLoc(E1), diag::err_cannot_infer_type, "vector-length on read"); OpError = true;
          } else if (!isa<VectorType>(T1)) {
              reportExpectedTypeError(getLoc(E1), VectorType::get({}), T1, "for vector-length operand"); OpError = true; // Expected type shown is illustrative
          }
          if (!OpError) resultType = IntegerType::get();
      } else { reportOperandCountError(1); }
      break;
  ...
  case TokenKind::vector_ref:
      if (E1 && E2 && !E3) {
          ...
          if (T1 == ReadPlaceholderType::get()) {
              reportError(getLoc(E1), diag::err_cannot_infer_type, "vector-ref on read"); OpError = true;
          } else if (!vecTy) {
              reportExpectedTypeError(getLoc(E1), VectorType::get({}), T1, "for vector-ref first operand"); OpError = true;
          }

          Int* indexNode = dyn_cast<Int>(E2);
          uint64_t indexVal = 0; // Use uint64_t for index
          if (!indexNode || indexNode->getValue().getAsInteger(10, indexVal)) { // Parse into uint64_t
              reportError(getLoc(E2), diag::err_vector_non_int_index, "Non-integer literal or parse error"); OpError = true;
          }

          if (!OpError) { // Proceed only if vector type and index are valid so far
              size_t index = static_cast<size_t>(indexVal); // Cast to size_t for vector access
              const auto& elementTypes = vecTy->getElementTypes();
              if (index >= elementTypes.size()) {
                   reportError(getLoc(E2), diag::err_vector_index_out_of_bounds, index, elementTypes.size());
                   OpError = true;
              } else {
  ...
  case TokenKind::vector_setb: // vector-set!
      if (E1 && E2 && E3) {
          VectorType* vecTy = dyn_cast<VectorType>(T1);
           if (T1 == ReadPlaceholderType::get()) {
              reportError(getLoc(E1), diag::err_cannot_infer_type, "vector-set! on read"); OpError = true;
          } else if (!vecTy) {
              reportExpectedTypeError(getLoc(E1), VectorType::get({}), T1, "for vector-set! first operand"); OpError = true;
          }
          ...
          // Infer read type if needed for the value being stored (E3)
           if (T3 == ReadPlaceholderType::get() && !OpError && vecTy) { // Check vecTy also
               ...
               if (index < elementTypes.size()) {
                   ...
                   if (index >= elementTypes.size() && !OpError) { // Avoid double reporting if index parse failed
                       reportError(getLoc(E2), diag::err_vector_index_out_of_bounds, index, elementTypes.size()); OpError = true;
                   } else if (!OpError){ // If index was valid but cannot infer T3
                       reportError(getLoc(E3), diag::err_cannot_infer_type, "read used as value for vector-set!"); OpError = true;
                   }
               }
           }
          ...
          if (!OpError) { // Proceed only if vector type and index are valid so far
              ...
              if (index >= elementTypes.size()) {
                  ...
                  if (!T3->equals(expectedType)) { // Check type match for the value
                       reportTypeError(getLoc(E3), expectedType, T3, "for vector-set! value");
                       OpError = true;
                  }
              }
          }
          if (!OpError) resultType = VoidType::get();
      } else { reportOperandCountError(3); }
      break;
